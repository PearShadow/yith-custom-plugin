...
jQuery( function ( $ ) {
	...
	var bkEnabledFields = {
		...
		init               : function () {
			...
			var downloadableField = $( '#_downloadable' ),
				virtualField      = $( '#_virtual' );
			...
			dom.productType.on( 'change', function () {
				var isBookableProduct = wcbk_admin.prod_type === $( this ).val();
				if ( isBookableProduct ) {
					if ( bkEnabledFields.initialProductType !== wcbk_admin.prod_type ) {
						...
						virtualField.attr( 'checked', 'checked' ).trigger( 'change' );
					}

					if ( downloadableField.is( ':checked' ) ) {
						downloadableField.attr( 'checked', false ).trigger( 'change' );
						dom.productType.trigger( 'change' ); // trigger change to product type select to call WC show_and_hide_panels()
					}
				}
