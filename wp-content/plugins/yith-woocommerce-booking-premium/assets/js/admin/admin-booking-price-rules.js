...
jQuery( function ( $ ) {
	...
	var yith_wcbk_price_rules = {
		...
		getBookingClasses       : function () {
			return yith_wcbk_price_rules.productData.attr( 'class' ).split( /\s+/ ).filter( function ( _ ) {
		...
		openAddRuleModal        : function ( event ) {
			...
			if ( ! yith_wcbk_price_rules.getRuleToAdd() ) {
				...
				var index   = yith_wcbk_price_rules.nextIndex(),
					classes = [ 'yith-wcbk-price-rules__create-modal' ].concat( yith_wcbk_price_rules.getBookingClasses() ).join( ' ' )
