...
( function ( $, wp ) {
	...
	var bkAvailabilityRules = {
		...
		getBookingClasses       : function () {
			return ( bkAvailabilityRules.dom.productData.length ) ? bkAvailabilityRules.dom.productData.attr( 'class' ).split( /\s+/ ).filter( function ( _ ) {
				return _.search( 'bk_' ) === 0
			} ) : '';
		},
		openAddRuleModal        : function ( event ) {
			...
			if ( ! bkAvailabilityRules.getRuleToAdd() ) {
				var index   = bkAvailabilityRules.nextIndex(),
					classes = [ 'yith-wcbk-availability-rules__create-modal' ].concat( bkAvailabilityRules.getBookingClasses() ).join( ' ' )
