	...
	$.fn.yith_booking_form = function () {
		...
		var onLoad                         = function () {
			...
			maybeLoadTimeSlots = function (options) {
                ...
                console.log('[maybeLoadTimeSlots] Starting: hasTime=', self.hasTime(), 'date=', date, 'duration=', duration);

                if ((self.hasTime() || self.isFullDay()) && date && duration) {
                    console.log('[maybeLoadTimeSlots] Conditions met, sending AJAX request');
                    postData.request = 'get_booking_available_times';

                    if (self.ajaxCall) {
                        console.log('[maybeLoadTimeSlots] Aborting previous AJAX call');
                        self.ajaxCall.abort();
                    }
                    ...
                    self.ajaxCall
                        .done(function (response) {
                            console.log('[maybeLoadTimeSlots] AJAX response received:', response);
                            var data = response.data;
                            try {
                                if (data.error) {
                                    console.log('[maybeLoadTimeSlots] Error in response:', data.error);
                                    self.dom.message.html('<p class="error">' + data.error + '</p>');
                                } else {
                                    if (data.time_data_html) {
                                        console.log('[maybeLoadTimeSlots] Populating time slots with:', data.time_data_html);
                                        self.dom.time.html(data.time_data_html);
                                    } else if (self.isFullDay()) {
                                        console.log('[maybeLoadTimeSlots] No time_data_html for full-day booking, using fallback');
                                        var timeOptions = '';
                                        for (var hour = 0; hour < 24; hour++) {
                                            var timeValue = (hour < 10 ? '0' : '') + hour + ':00';
                                            timeOptions += '<option value="' + timeValue + '">' + timeValue + '</option>';
                                        }
                                        self.dom.time.html(timeOptions);
                                    }
                                    if (time) {
                                        console.log('[maybeLoadTimeSlots] Restoring selected time:', time);
                                        var $option_selected = self.dom.time.find('option[value="' + time + '"]');
                                        if ($option_selected.length) {
                                            $option_selected.attr('selected', 'selected');
                                            console.log('[maybeLoadTimeSlots] Selected time restored:', time);
                                        } else {
                                            console.log('[maybeLoadTimeSlots] Selected time not found in options:', time);
                                        }
                                    }
                                    if (data.message) {
                                        console.log('[maybeLoadTimeSlots] Setting message:', data.message);
                                        self.dom.message.html('<p>' + data.message + '</p>');
                                    }
                                    console.log('[maybeLoadTimeSlots] Triggering select-list update');
                                    self.dom.time.trigger('yith-wcbk-select-list:update');
                                    self.form.trigger('yith_wcbk_form_update_time', data);
                                }
                            } catch (err) {
                                console.log('[maybeLoadTimeSlots] Error processing response:', err);
                            }
                        })
                        .always(function () {
                            console.log('[maybeLoadTimeSlots] AJAX complete, validating form');
                            var data = validateForm();
                            if (!options.updateFormOnCompleteOnlyIfValid || data.validation) {
                                console.log('[maybeLoadTimeSlots] Triggering form update');
                                self.form.trigger('yith_wcbk_booking_form_update');
                            } else {
                                console.log('[maybeLoadTimeSlots] Form update skipped due to invalid form');
                            }
                        });
                } else {
                    console.log('[maybeLoadTimeSlots] Conditions not met: hasTime=', self.hasTime(), 'isFullDay=', self.isFullDay(), 'date=', date, 'duration=', duration);
                }

                console.log('[maybeLoadTimeSlots] Returning loaded:', loaded);
                return loaded;
            },
