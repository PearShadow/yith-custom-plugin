...
if ( ! class_exists( 'YITH_WCBK_Post_Types' ) ) {
	...
	class YITH_WCBK_Post_Types {
		...
		public static function init() {
			...
			add_filter( 'product_type_selector', array( __CLASS__, 'add_bookable_product_type' ) );

			add_filter( 'woocommerce_data_stores', array( __CLASS__, 'register_data_stores' ), 10, 1 );

			self::include_admin_handlers();
		...
		public static function register_post_status() {
			$singulars = apply_filters(
				'yith_wcbk_booking_cpt_singular_labels',
				array(
			...
			$plurals   = apply_filters(
				'yith_wcbk_booking_cpt_plurals_labels',
				array(
		...
		/**
		 * Add the bookable product type to WC product types.
		 *
		 * @param array $types Product types.
		 *
		 * @return array
		 */
		public static function add_bookable_product_type( $types ) {
			$types[ self::PRODUCT_TYPE ] = _x( 'Bookable Product', 'Admin: type of product', 'yith-booking-for-woocommerce' );
		...
		public static function save_meta_boxes( $post_id, $post ) {
			...
			if ( in_array( $post->post_type, self::get_post_types(), true ) ) {
				...
				/**
				 * DO_ACTION: yith_wcbk_post_process_{$key}_meta
		...
		public static function get_post_types() {
			/**
			 * APPLY_FILTERS: yith_wcbk_post_types
			 *
