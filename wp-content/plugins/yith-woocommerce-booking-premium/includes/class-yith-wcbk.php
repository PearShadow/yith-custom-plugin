...
if ( ! class_exists( 'YITH_WCBK' ) ) {
	...
	class YITH_WCBK {
		...
		/**
		 * Return the Reserved Bookings class instance.
		 *
		 * @return YITH_WCBK_Reserved_Bookings
		 */
		public function reserved_bookings() {
			return YITH_WCBK_Reserved_Bookings::get_instance();
		}
			...
			if ( isset( $caps[0] ) ) {
				switch ( $caps[0] ) {
					case 'view_booking':
						...
						/**
						 * APPLY_FILTERS: yith_wcbk_user_can_view_booking
						 *
						 * Filters if the specific user has permissions to view booking and its details.
						 *
						 * @param boolean           $can     True if the user has permissions to view; false otherwise.
		...
		public function is_request( $type ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_is_request
			 *
