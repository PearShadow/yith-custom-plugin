...
class YITH_WCBK_Request_A_Quote_Integration extends YITH_WCBK_Integration {
	...
	public function add_booking_data_in_raq_emails( $item ) {
		if ( $item instanceof WC_Order_Item_Product ) {
			$item = array(
				...
				'yith_booking_request' => $item->get_meta( 'yith_booking_request' ),
			);
		}
	...
	public function add_booking_to_quote( $raq, $product_raq ) {
		...
		if ( $product_id ) {
			...
			if ( $product && $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ) {
	...
	public function add_custom_scripts() {
		if ( is_product() ) {
			...
			if ( $hide_price && $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ) {
