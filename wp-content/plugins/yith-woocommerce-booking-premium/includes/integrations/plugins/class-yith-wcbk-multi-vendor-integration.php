...
class YITH_WCBK_Multi_Vendor_Integration extends YITH_WCBK_Integration {
	...
	protected function init() {
		...
		if ( $this->is_enabled() ) {
			...
			add_filter( 'yith_wcbk_orders_get_order_item_booking_id', array( $this, 'filter_booking_id_from_suborder_related_order_item' ), 10, 2 );
	...
	/**
	 * Filter booking ID, to get the one of the related suborder item.
	 *
	 ...
	 * @since 5.9.1
	 */
	public function filter_booking_id_from_suborder_related_order_item( $booking_id, $order_item ) {
		if ( ! $booking_id ) {
			$order = $order_item->get_order();
			if ( $order && ! ! $order->get_meta( 'has_sub_order' ) && is_callable( 'YITH_Vendors_Orders::get_child_item_id' ) ) {
				$child_item_id = YITH_Vendors_Orders::get_child_item_id( absint( $order_item->get_id() ) );
				if ( $child_item_id ) {
					$child_item = WC_Order_Factory::get_order_item( absint( $child_item_id ) );

					return $child_item->get_meta( '_booking_id' );
				}
			}
		}
	...
	public function remove_booking_in_product_type_selector_for_vendors( $types ) {
		$vendor = $this->get_current_vendor();
		if ( $vendor && isset( $types[ YITH_WCBK_Post_Types::PRODUCT_TYPE ] ) ) {
			unset( $types[ YITH_WCBK_Post_Types::PRODUCT_TYPE ] );
		}
