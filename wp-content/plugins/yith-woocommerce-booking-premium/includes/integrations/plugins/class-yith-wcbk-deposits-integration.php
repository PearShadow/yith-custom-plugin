...
class YITH_WCBK_Deposits_Integration extends YITH_WCBK_Integration {
	...
	protected function init() {
		...
		add_filter( 'yith_wcdp_loop_add_deposit_to_cart_args', array( $this, 'add_booking_query_arg_to_add_deposit_url' ), 10, 2 );
	...
	public function add_custom_expiration_settings( $settings ) {
		...
		if ( 'yith_wcdp_admin_product_fields' === $current_action ) {
			...
			if ( isset( $field_to_add['id'] ) ) {
				unset( $field_to_add['id'] );
			}
			...
			$key = version_compare( YITH_WCDP::YITH_WCDP_VERSION, '2.23.0', '>=' ) ? 'balances' : 'settings-balances';

			$settings[ $key ] = yith_wcdp_append_items(
				$settings[ $key ],
	...
	/**
	 * Filter Pay Deposit link in case of Booking Search Form lost.
	 *
	 * @param array      $args Query args for pay deposit link.
	 * @param WC_Product $product Current product.
	 *
	 * @return mixed
	 */
	public function add_booking_query_arg_to_add_deposit_url( $args, $product ) {
		if ( class_exists( 'YITH_WCBK_Search_Forms_Frontend' ) ) {
			$args['product_url'] = YITH_WCBK_Search_Forms_Frontend::get_instance()->add_booking_data_in_search_result_links( $args['product_url'], $product );
		}
