...
if ( ! class_exists( 'YITH_WCBK_Booking_Post_Type_Admin' ) ) {
	...
	class YITH_WCBK_Booking_Post_Type_Admin extends YITH_WCBK_Post_Type_Admin {
		...
		protected function is_the_list_filtered() {
			...
			return ! empty( $_GET['date_from'] ) || ! empty( $_GET['date_to'] );
		}
		...
		public function render_columns( $column, $post_id ) {
			...
			if ( $this->object ) {
				/**
				 * DO_ACTION: yith_wcbk_booking_render_custom_columns
		...
		public function maybe_render_blank_state( $which ) {
			...
			if ( $this->get_blank_state_params() && $post_type === $this->post_type && 'bottom' === $which ) {
				...
				if ( $show_blank_state ) {
					$css = '#posts-filter .wp-list-table, #posts-filter .yith-plugin-ui__wp-list-auto-h-scroll__wrapper, .tablenav.bottom > * { display : none; } #posts-filter .tablenav.bottom { height  : auto; display : block }';
					if ( ! $show_views ) {
