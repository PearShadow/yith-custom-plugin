...
if ( ! class_exists( 'YITH_WCBK_AJAX' ) ) {
	...
	class YITH_WCBK_AJAX {
		...
		public function add_booking_note() {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_note
			 *
		...
		public function json_search_order() {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_json_search_order_term
			 *
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_json_search_order_limit
			 *
			...
			if ( $orders ) {
				...
				foreach ( $orders as $order ) {
					...
					/**
					 * APPLY_FILTERS: yith_wcbk_order_number
					 *
		...
		public function json_search_booking_products() {
			...
			if ( $booking_term ) {
				...
				if ( count( $posts_in ) > 0 ) {
					...
					/**
					 * APPLY_FILTERS: yith_wcbk_json_search_booking_products_args
					 *
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_json_search_found_booking_products
			 *
			 * Filters the bookable products before returning them when searching for them in AJAX.
			 *
			 * @param array $found_products The products' key-value array (key is the product id, value is the formatted product name).
			 *
		...
		public function get_product_booking_form() {
			...
			if ( isset( $_POST['product_id'] ) ) {
				...
				/**
				 * DO_ACTION: yith_wcbk_booking_form
		...
		public function frontend_ajax_get_booking_data( $request = false ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_ajax_booking_data_request
			 *
			...
			if ( empty( $request['product_id'] ) || empty( $request['from'] ) || ( empty( $request['duration'] ) && empty( $request['to'] ) ) ) {
				...
				if ( $product ) {
					...
					$exclude_order        = $this->get_draft_or_resuming_order();
					if ( $exclude_order ) {
						$is_available_args['exclude_order_id'] = $exclude_order->get_id();
					}

					/**
					 * APPLY_FILTERS: yith_wcbk_product_form_get_booking_data_available_args
					 *
					 * Filters the args used to check for availability when retrieving booking data in AJAX.
					 *
					 * @param array              $is_available_args The arguments.
					 * @param WC_Product_Booking $product           The bookable product.
					 * @param array              $request           The request.
					 *
					 * @return array
					...
					if ( $is_available ) {
						...
						/**
						 * APPLY_FILTERS: yith_wcbk_product_form_not_bookable_price_html
						 *
						 * Filters the price to be shown in the booking form when the product is non-bookable.
						 *
						 * @param string $price_html    The price HTML.
						 * @param array  $request       The request.
						 * @param array  $bookable_args The arguments.
						 *
						 * @return string
						 */
					...
					/**
					 * APPLY_FILTERS: yith_wcbk_product_form_get_booking_data
					 *
					 * Filters the booking data retrieved in AJAX.
					 *
					 * @param array              $booking_data  The data.
					 * @param WC_Product_Booking $product       The bookable product.
					 * @param array              $bookable_args The arguments.
					 * @param array              $request       The request.
					 *
		...
		protected function frontend_ajax_get_booking_available_times() {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_ajax_booking_available_times_request
			 *
			...
			if ( empty( $request['product_id'] ) || empty( $request['from_date'] ) || empty( $request['duration'] ) ) {
				...
				/**
				 * APPLY_FILTERS: yith_wcbk_ajax_booking_available_times_availability_args
				 *
				...
				if ( $product ) {
					...
					if ( ! $time_data ) {
                        $data['time_data_html'] = '<option value="">' . apply_filters('yith_wcbk_time_data_html_label', __( 'No time available', 'yith-booking-for-woocommerce' ), $product )  . '</option>';
					}
				}
			}
		...
		protected function frontend_ajax_get_product_non_available_dates() {
			...
			if ( empty( $request['product_id'] ) ) {
				...
				if ( $product ) {
					...
					/**
					 * APPLY_FILTERS: yith_wcbk_ajax_booking_non_available_dates_availability_args
					 *
					 * Filters the availability args when retrieving the non-available dates in AJAX.
					 *
					 * @param array $availability_args The args.
					 * @param array $request           The request.
					 *
					 * @return array
					 */
