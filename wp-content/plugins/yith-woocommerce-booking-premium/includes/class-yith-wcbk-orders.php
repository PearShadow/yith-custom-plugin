...
if ( ! class_exists( 'YITH_WCBK_Orders' ) ) {
	...
	class YITH_WCBK_Orders {
		...
		/**
		 * Get the booking ID related to a specific order item.
		 *
		 * @param WC_Order_Item $order_item The order item.
		 * @param string        $context    The context (edit, view).
		 *
		 * @return int
		 * @since 5.9.1
		 */
		public function get_order_item_booking_id( $order_item, $context = 'view' ) {
			$booking_id = $order_item->get_meta( '_booking_id' );

			if ( 'view' === $context ) {
				$booking_id = apply_filters( 'yith_wcbk_orders_get_order_item_booking_id', $booking_id, $order_item );
			}
		...
		public function print_order_related_bookings_meta_box( $post ) {
			$order = wc_get_order( $post instanceof WP_Post ? $post->ID : $post );
			if ( $order ) {
				$bookings = yith_wcbk()->booking_helper->get_bookings_by_order( $order->get_id() );
				/**
				 * APPLY_FILTERS: yith_wcbk_order_bookings_related_to_order
				 *
				 * Filters the bookings related to a specific order
				 *
		...
		public function show_related_bookings( $order ) {
			...
			if ( ! ! $bookings ) {
				/**
				 * APPLY_FILTERS: yith_wcbk_related_booking_title
				 *
				 ...
				 * @return string
				 */
				$title = apply_filters( 'yith_wcbk_related_booking_title', __( 'Related Bookings', 'yith-booking-for-woocommerce' ) );
		...
		public function get_booking_order_item_details( $booking_data, $product_id = 0 ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_order_get_booking_order_item_details
			 *
		...
		public function check_order_for_booking( $order, $deprecated = array() ) {
			/**
			 * APPLY_FILTERS: yith_wcbk_order_check_order_for_booking
			 *
			 * Filters if checking the order for bookings.
			 *
			 * @param bool  $check      True to check; false otherwise.
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_order_check_order_for_booking_order
			 *
			 * Filters the order to take into account when checking for creating bookings.
			 *
			 * @param WC_Order|false $order The order
			 *
			 * @return WC_Order|false
			 */
			$order = apply_filters( 'yith_wcbk_order_check_order_for_booking_order', $order );
			...
			foreach ( $order_items as $order_item_id => $order_item ) {
				if ( $order_item->is_type( 'line_item' ) ) {
					/**
					 * APPLY_FILTERS: yith_wcbk_order_check_order_item_for_booking
					 *
					...
					$the_booking_id = $this->get_order_item_booking_id( $order_item, 'edit' );
					...
					if ( ! $the_booking_id && ! ! $booking_data && isset( $booking_data['from'] ) ) {
						...
						/**
						 * APPLY_FILTERS: yith_wcbk_booking_product_id_to_translate
						 *
						 * Filters the product id to be translated (if you need to translate it).
						 *
						 * @param int $product_id The args.
						 *
						 * @return int
						 */
						...
						foreach ( $all_props as $prop => $value ) {
							if ( $booking->is_internal_prop( $prop ) ) {
								...
								/**
								 * APPLY_FILTERS: yith_wcbk_force_private_meta_data_on_booking_creation
								 *
								 * Filters if forcing private meta-data on booking creating in Orders.
								 *
								 * @param bool   $force True to force private meta-data; false otherwise.
						...
						if ( $booking->get_id() ) {
							...
							/**
							 * DO_ACTION: yith_wcbk_order_booking_created
						...
						/**
						 * APPLY_FILTERS: yith_wcbk_orders_associate_order_item_booking_to_order
						 *
						 * Filters if associating order data to a newly created booking.
						 *
						 * @param bool              $associate  True to associate; false otherwise.
						 * @param YITH_WCBK_Booking $booking    The booking.
						 * @param WC_Order          $order      The order.
						 * @param WC_Order_Item     $order_item The order_item.
						 *
						 * @return bool
						 */
		...
		public function set_booking_as_paid( $order_id, $order ) {
			/**
			 * APPLY_FILTERS: yith_wcbk_orders_set_booking_as_paid
			 *
			 * Filters if setting booking as paid (when setting the order to 'processing' or 'completed').
			 *
			 * @param bool     $set_as_paid True to set as paid; false otherwise.
			 * @param int      $order_id    Order ID.
			 * @param WC_Order $order       The Order.
			 *
			 * @return bool
			...
			if ( ! ! apply_filters( 'yith_wcbk_orders_set_booking_as_paid', true, $order_id, $order ) ) {
				...
				if ( ! ! ( $bookings ) ) {
					foreach ( $bookings as $booking ) {
						/**
						 * APPLY_FILTERS: yith_wcbk_orders_should_set_booking_as_paid
						 *
