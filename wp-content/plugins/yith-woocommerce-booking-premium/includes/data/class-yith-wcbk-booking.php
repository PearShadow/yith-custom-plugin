	...
	class YITH_WCBK_Booking extends YITH_WCBK_Booking_Abstract {
		...
		*/
		...
		public function get_person_types_html( string $format = '', string $join = '<br />' ): string {
			...
			foreach ( $this->get_person_types() as $person_type ) {
				...
				$person_type_title = yith_wcbk()->person_type_helper()->get_person_type_title( $id );
		...
		public function get_product() {
			if ( ! isset( $this->related_objects['product'] ) ) {
				...
				$this->related_objects['product'] = $product && $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ? $product : false;
			}
		...
		*/
		...
		protected function status_transition() {
			...
			if ( $status_transition ) {
				try {
					...
					/**
					 * DO_ACTION: yith_wcbk_booking_status_{$status_to}
					 * Hook to perform any action on booking status transition.
					 *
					 * @param bool              $status_to The new status.
					...
					if ( $status_from ) {
						...
						/**
						 * DO_ACTION: yith_wcbk_booking_status_{$status_from}_to_{$status_to}
						 * Hook to perform any action on booking status transition.
						 *
						 * @param int               $id      The booking ID.
						 * @param YITH_WCBK_Booking $booking The booking.
						...
						/**
						 * DO_ACTION: yith_wcbk_booking_status_changed
						 * Hook to perform any action on booking status transition.
						 *
						 * @param int               $id          The booking ID.
		...
		public function get_time_to_start_html() {
			...
			return '<div class="yith-wcbk-booking-time-to-start yith-wcbk-booking-time-to-start--' . esc_attr( $details['status'] ) . '">' . esc_html( $label ) . '</div>';
		}
