	...
	class WC_Product_Booking extends YITH_WCBK_Legacy_Booking_Product {
		...
		*/
		...
		public function get_price( $context = 'view' ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_product_get_price
			 *
		...
		public function get_price_html( $deprecated = '' ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_product_get_price_html
			 *
		...
		*/
		...
		public function is_available( $args = array() ) {
			/**
			 * DO_ACTION: yith_wcbk_booking_before_is_available
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_product_is_available_args
			 *
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_is_available_data_static
			 *
			 * Filters the available data when checking for the availability of a bookable product.
			 *
			 * @param array              $available_data The available data.
			 * @param array              $args           Arguments.
			 * @param WC_Product_Booking $product        The bookable product.
			 *
			 * @return array
			 ...
			 *
			 * @param array              $non_available_reasons Non-available reasons.
			 * @param array              $args                  Arguments.
			 * @param WC_Product_Booking $product               The bookable product.
			 * @param int                $_remained             The number of seats remained.
			 *
			 * @return array
			 */
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_is_available_data
			 *
			 * Filters the available data when checking for the availability of a bookable product.
			 *
			 * @param array              $available_data The available data.
		...
		public function is_sold_individually() {
			return apply_filters( 'yith_wcbk_booking_product_is_sold_individually', true, $this );
		}
		...
		public function is_virtual() {
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_product_is_virtual
			 *
			 * Filters if the bookable product is virtual.
			 *
			 * @param boolean            $is_virtual Is virtual flag.
			 * @param WC_Product_Booking $product    The bookable product.
			 *
			 * @return boolean
			 */
		...
		public function has_time_slot_available_in_range( $product, $date, $from_time, $to_time ) {
    		$time_slots = $product->get_meta( '_yith_booking_time_slots' ) ?: array();
    		foreach ( $time_slots as $slot ) {
        		if (
            		strtotime( $slot['start'] ) >= strtotime( $from_time ) &&
            		strtotime( $slot['end'] ) <= strtotime( $to_time ) &&
            		$product->is_time_slot_available( $date, $slot ) // Hypothetical method
        		) {
            		return true;
        		}
    		}
		...
		*/
		...
		public function calculate_cost( $timestamp, $type = 'base_price', $args = array() ) {
			/**
			 * DO_ACTION: yith_wcbk_booking_before_calculate_cost
			 * Hook to perform any action before calculating the bookable product cost.
			 *
			 * @param int                $timestamp The timestamp for which the cost is calculated.
			 * @param string             $type      The type of the cost.
			 * @param WC_Product_Booking $product   The bookable product.
			 */
		...
		public function parse_price_args( $args = array() ) {
			...
			// Round duration to the next full day if an hour is above a full day (24 hours).
			$duration_unit = $this->get_duration_unit();
			if ( 'hour' === $duration_unit ) {
				$hours_per_day = 24 / $this->get_duration(); // Number of duration units in a day (e.g., 24 if duration is 1 hour).
				$full_days = floor( $args['duration'] / $hours_per_day ); // Number of complete 24-hour days
				$remaining_hours = $args['duration'] % $hours_per_day; // Remaining hours after complete days

				if ( $remaining_hours > 0 ) {
					// If there are any hours beyond a full day, round up to the next full day
					$args['duration'] = ($full_days + 1) * $hours_per_day;
				} else {
					// If exactly on a full day boundary, keep as is
					$args['duration'] = $full_days * $hours_per_day;
				}
			} elseif ( 'day' === $duration_unit ) {
				// For day-based units, treat <= 1 day as 1 day; > 1 day uses actual duration.
				if ( $args['duration'] <= 1 ) {
					$args['duration'] = 1;
				}
			}
			// Add support for other units (e.g., minute, month) if needed.
		...
		public function calculate_price( $args = array() ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_booking_product_calculated_price
			 *
