...
if ( ! class_exists( 'YITH_WCBK_Product_Post_Type_Admin' ) ) {
	...
	class YITH_WCBK_Product_Post_Type_Admin {
		...
		/**
		 * The bookable product type
		 *
		 * @var string
		 * @static
		 * @deprecated 5.14.2 | use YITH_WCBK_Post_Types::PRODUCT_TYPE instead
		 */
		public static $prod_type = YITH_WCBK_Post_Types::PRODUCT_TYPE;
		...
		public function add_product_data_panels() {
			...
			$post_id         = $post->ID;
			$prod_type       = YITH_WCBK_Post_Types::PRODUCT_TYPE;
			$booking_product = $product_object->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ? $product_object : false;
		...
		public function product_booking_tabs( $tabs ) {
			...
			$booking_tabs = array(
				'yith_booking_options' => array(
					...
					'class'    => array( 'show_if_' . YITH_WCBK_Post_Types::PRODUCT_TYPE ),
			...
			foreach ( $sub_tabs as $key => $sub_tab ) {
				...
				$class    = array_merge( (array) $class, array( 'yith-wcbk-product-sub-tab', 'show_if_' . YITH_WCBK_Post_Types::PRODUCT_TYPE ) );
		 ...
		 *
		 * @return array
		 * @deprecated 5.14.2 | use YITH_WCBK_Post_Types::add_bookable_product_type instead
		 */
		public function product_type_selector( $types ) {
			yith_wcbk_deprecated_function( 'YITH_WCBK_Product_Post_Type_Admin::product_type_selector', '5.14.2', 'YITH_WCBK_Post_Types::add_bookable_product_type' );

			return YITH_WCBK_Post_Types::add_bookable_product_type( $types );
		}
		...
		public function product_type_options( $options ) {
			$options['virtual']['wrapper_class'] .= ' show_if_' . YITH_WCBK_Post_Types::PRODUCT_TYPE;
		...
		public function set_product_meta_before_saving( $product ) {
			...
			if ( $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ) {
				...
				/**
				 * DO_ACTION: yith_wcbk_process_bookable_product_meta
		...
		public static function is_booking( $product = null ) {
			...
			if ( $product instanceof WC_Product ) {
				return $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE );
			...
			return YITH_WCBK_Post_Types::PRODUCT_TYPE === $product_type;
		}
	}
}
