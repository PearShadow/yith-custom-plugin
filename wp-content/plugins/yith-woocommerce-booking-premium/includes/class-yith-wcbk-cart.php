...
if ( ! class_exists( 'YITH_WCBK_Cart' ) ) {
	...
	class YITH_WCBK_Cart {
		...
		public static function get_booking_data_from_request( $request = array() ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cart_booking_data_request{$object_type}_id
			 *
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cart_get_booking_data_from_request
			 *
		...
		public static function get_booking_props_from_booking_data( array $booking_data = array() ): array {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cart_get_booking_props_from_booking_data
			 *
		...
		public static function get_booking_data_from_booking( YITH_WCBK_Booking $booking ): array {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cart_get_booking_data_from_booking
			 *
		...
		public static function get_availability_args_from_booking_data( array $booking_data = array() ): array {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cart_get_availability_args_from_booking_data
			 *
		...
		public function add_to_cart_validation( $passed_validation, $product_id, $quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
			if ( yith_wcbk_is_booking_product( $product_id ) ) {
				...
				if ( $product ) {
					...
					if ( ! $booking_data ) {
						...
						if ( $passed_validation && $product->get_max_bookings_per_unit() ) {
							...
							if ( $total_bookings + $booking_weight > $max_booking_per_block ) {
								...
								/**
								 * APPLY_FILTERS: yith_wcbk_no_add_to_cart_for_selected_data
								 *
								 ...
								 * @param string $remained_text The text explaining how much "seats" are available.
								 *
								 * @return string
								 */
						...
						/**
						 * DO_ACTION: yith_wcbk_after_add_to_cart_validation
						 * Hook to perform any action after add-to-cart validation.
						 *
						 * @param WC_Product_Booking $product           The bookable product.
						 * @param bool               $passed_validation True if passed validation; false otherwise.
						 * @param array              $booking_data      The booking data.
						 * @param array              $cart_item_data    The cart item data.
						 */
					...
					/**
					 * APPLY_FILTERS: yith_wcbk_add_to_cart_validation
					 *
		...
		protected function get_cart_errors_for_booking_availability(): array {
			...
			foreach ( $cart->get_cart() as $cart_item_key => $values ) {
				...
				if ( $product && $product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) && $booking_data ) {
					$availability_args = self::get_availability_args_from_booking_data( $booking_data );
					$exclude_order     = $this->get_draft_or_resuming_order();
					if ( $exclude_order ) {
						$availability_args['exclude_order_id'] = $exclude_order->get_id();
					}
		...
		public function woocommerce_get_item_data( $item_data, $cart_item ) {
			...
			if ( yith_wcbk_is_booking_product( $product_id ) ) {
				...
				/**
				 * APPLY_FILTERS: yith_wcbk_cart_booking_item_data_before_totals
				 *
				 * Filters the item data related to a bookable product item in cart (before the totals item data).
				 *
				 * @param array              $booking_item_data The  item data.
				 * @param array              $cart_item         The cart item.
				 * @param WC_Product_Booking $product           The bookable product.
				 *
				 * @return array
