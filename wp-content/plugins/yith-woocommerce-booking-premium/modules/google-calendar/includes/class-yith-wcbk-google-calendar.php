		...
		public function sync_booking_event( $booking ) {
			...
			if ( $this->is_calendar_sync_enabled() ) {
				...
				if ( $booking && $booking->is_valid() ) {
					...
					$query_parameters = apply_filters( 'yith_wcbk_google_calendar_sync_event_query_parameters', array(), $event_args, $booking );

					if ( $query_parameters ) {
						$event_uri = add_query_arg( $query_parameters, $event_uri );
					}
