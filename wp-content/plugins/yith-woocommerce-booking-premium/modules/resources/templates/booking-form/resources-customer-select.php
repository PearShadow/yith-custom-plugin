...
foreach ( $resources_data as $resource_data ) {
	...
	if ( $resource ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_resources_selector_resource_description
		 *
		 * Filters the resource description shown in the resources' selector.
		 *
		 * @param string                  $description   The description (by default it'll be the pricing).
		 * @param YITH_WCBK_Resource      $resource      The resource.
		 * @param YITH_WCBK_Resource_Data $resource_data The resource data.
		 * @param WC_Product_Booking      $product       The bookable product.
		 *
		 * @return string
		 */
		$description = apply_filters( 'yith_wcbk_resources_selector_resource_description', $description, $resource, $resource_data, $product );
