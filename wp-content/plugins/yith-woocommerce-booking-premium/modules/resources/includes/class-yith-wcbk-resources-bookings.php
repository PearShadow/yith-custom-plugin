...
if ( ! class_exists( 'YITH_WCBK_Resources_Bookings' ) ) {
	...
	class YITH_WCBK_Resources_Bookings {
		...
		protected function __construct() {
			...
			add_action( 'yith_wcbk_admin_calendar_action_bar_reset_filters_url', array( $this, 'filter_calendar_reset_filters_url' ) );
		...
		public function filter_booking_data_to_display( array $data, string $context, array $args, YITH_WCBK_Booking $booking ): array {
			...
			if ( $resource_ids ) {
				...
				if ( $is_admin ) {
					...
					$show = apply_filters( 'yith_wcbk_resources_show_resources_in_booking_frontend_data', in_array( $resource_assignment, array( 'customer-select-one', 'customer-select-more' ), true ), $product, $booking );
					if ( $show ) {
						$value = implode(
							', ',
							array_map(
		...
		/**
		 * Filter the "reset filters" URL in calendar, to show the button if a resource is selected to filter the calendar.
		 *
		 * @param string|false $url The URL.
		 *
		 * @return string|false
		 * @since 5.9.1
		 */
		public function filter_calendar_reset_filters_url( $url ) {
			$resource = absint( $_REQUEST['resource'] ?? 0 ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			return $resource ? remove_query_arg( 'resource', $url ) : $url;
		}
