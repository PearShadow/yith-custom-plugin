...
if ( ! class_exists( 'YITH_WCBK_ICS_Parser' ) ) {
	...
	class YITH_WCBK_ICS_Parser {
		...
		public function ical_date_to_timestamp( $ical_date ) {
			...
			if ( $is_utc_time ) {
				$timezone_offset = $this->get_timezone_offset_for_utc_date( (int) $date[1], (int) $date[2], (int) $date[3] );
		...
		/**
		 * Retrieve the timezone offset for a specific date.
		 *
		 ...
		 * @return float|int
		 */
		private function get_timezone_offset_for_utc_date( $year, $month, $day ) {
