...
if ( 'month' !== $unit ) {
	if ( yith_wcbk()->settings->is_unique_calendar_range_picker_enabled() && $product->has_calendar_picker_enabled() ) {
		...
		if ( $product->has_calendar_picker_enabled() ) {
			...
			wc_get_template( '/single-product/add-to-cart/booking-form/dates/ui-end-date.php', compact( 'product', 'date_info', 'not_available_dates', 'calendar_day_range_picker_class' ), '', YITH_WCBK_TEMPLATE_PATH );
		}
		...
		if ( in_array( $unit, array( 'hour', 'minute' ), true ) ) {
			...
			wc_get_template( '/single-product/add-to-cart/booking-form/dates/ui-end-time.php', compact( 'product' ), '', YITH_WCBK_TEMPLATE_PATH );
		}
	}
} else {
