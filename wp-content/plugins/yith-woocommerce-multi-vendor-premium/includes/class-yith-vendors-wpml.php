...
if ( ! class_exists( 'YITH_Vendors_WPML' ) ) {
	...
	class YITH_Vendors_WPML {
		...
		protected function init() {
			...
			add_filter( 'yith_wcmv_get_vendor_edit_data', array( $this, 'filter_vendor_edit_data' ), 15, 4 );
		...
		protected function get_vendor_translated( $vendor_id, $language = '' ) {
			global $yith_wcmv_cache, $sitepress;
			...
			// Remove sitepress filter to make sure get the correct taxonomy object.
		...
		/**
		 * Filter vendor edit data (admin modal)
		 *
		 * @since 5.4.0
		 * @param array       $data   The vendor data array key => value.
		 * @param YITH_Vendor $vendor The vendor object.
		 * @param boolean     $modal  True if the request comes from AJAX modal, false otherwise.
		 * @param array       $fields The fields to get data for.
		 */
		public function filter_vendor_edit_data( $data, $vendor, $modal, $fields ) {

			// Exit if vendor is not a translation.
			if ( ! $this->is_vendor_a_translation( $vendor->get_id() ) ) {
				return $data;
			}

			$original_vendor = $this->get_vendor_translated( $vendor->get_id(), $this->default_language );

			if ( empty( $original_vendor ) ) {
				return $data;
			}

			$original_vendor_data = YITH_Vendors_Factory::get_data( $original_vendor, $modal, $fields );

			error_log( print_r( $original_vendor_data, true ) );

			array_walk(
				$data,
				function ( &$value, $key ) use ( $original_vendor, $original_vendor_data ) {
					if ( ! in_array( $key, $this->get_translatable_keys(), true ) && $original_vendor->is_meta( $key ) && 'vendor_id' !== $key ) {
						$value = $original_vendor_data[ $key ];
					}
				}
			);
