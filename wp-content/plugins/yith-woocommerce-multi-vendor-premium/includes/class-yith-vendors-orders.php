...
if ( ! class_exists( 'YITH_Vendors_Orders' ) ) {
	...
	class YITH_Vendors_Orders extends YITH_Vendors_Orders_Legacy {
		...
		protected function register_actions() {
			...
			// Handle special cap.
			add_filter( 'user_has_cap', array( $this, 'handle_edit_shop_order_cap' ), 10, 3 );
		...
		/**
		 * Handle special order cap edit_shop_order
		 *
		 * @param array $all_caps An array of caps.
		 * @param array $requested_caps An array of requested caps.
		 * @param array $args Caps arguments.
		 * @return array
		 */
		public function handle_edit_shop_order_cap( $all_caps, $requested_caps, $args ) {
			if ( 'yes' !== get_option( 'yith_wpv_vendors_option_order_management', 'no' ) || ! in_array( 'edit_shop_order', $args, true ) ) {
				return $all_caps;
			}
			...
			if ( empty( $order ) || empty( $vendor ) || ! $vendor->is_valid() || $vendor->get_id() !== (int) $order->get_meta( 'vendor_id' ) ) {
				return $all_caps;
			}

			$requested_caps = array_combine( $requested_caps, array_fill( 0, count( $requested_caps ), true ) );
