...
if ( ! class_exists( 'YITH_Vendors_Factory' ) ) {
	...
	class YITH_Vendors_Factory {
		 ...
		 *
		 * @since  4.0.0
		 * @param YITH_Vendor|integer $vendor The vendor object or id to get data for.
		 ...
		 */
		public static function get_data( $vendor, $modal = true, $fields = array() ) {

			try {

				$vendor = self::read( $vendor );
				if ( ! $vendor || ! $vendor->is_valid() ) {
					throw new Exception( sprintf( 'Invalid vendor. No vendor with ID #%s', $vendor ) );
				}
